...
def eval_sufficiency(ff, input_embed, position_embed, type_embed, attention_mask, mask_token_emb, attr, topk=20):
	...
	logits_perturbed						= ff(masked_input_embed, attention_mask=masked_attention_mask, position_embed=masked_position_embed, type_embed=masked_type_embed, return_all_logits=True).squeeze()
	prob_perturbed							= torch.softmax(logits_perturbed, dim=0)
